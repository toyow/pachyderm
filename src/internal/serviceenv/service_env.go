...
// the lifetime of the ServiceEnv. Connections used by these clients are shared
// by all callers after they are initialized (per gRPC best practice).
//
// In pachd, there is only one instance of this struct, but tests may create
// multiple, to give each test its own pachyderm "cluster" served in a separate
// goroutine.
